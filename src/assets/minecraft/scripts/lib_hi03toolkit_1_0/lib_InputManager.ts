...
export class InputManager {
    ...
    register(name: string, keyCode: number, useOptionKey: boolean, description:string): void {
        this.entries[name] = {
            keyCode: keyCode,
    ...
    update(): void {
        for (const key in this.entries) {
            ...
            entry.current = entry.useOptionKey ? keyDown && optionDown : keyDown && !optionDown;
            if (!entry.previous && entry.current) entry.pressedTime = System.currentTimeMillis();
