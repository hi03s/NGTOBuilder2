...
function init(par1: ModelSetVehicle, par2: ModelObject): void {
    ...
    keyManager.register("beamDistanceDecrease", Keyboard.KEY_DOWN, false, "ワイヤー式ビームの長さを0.1m減らす");
...
function renderForToolUser(entity: EntityVehicle, pass: number, par3: number): void {
    ...
    if (lookingPos) {
        ...
        if (lookingRailMap) {
            ...
            const insulatorOffset = isDeviation ? collectorList[0].size(entity) % 2 === 0 !== isDeviationInvert ? insulatorOffset1 : insulatorOffset2 : insulatorOffset1;
...
function rebuildParallelLane(entity: EntityVehicle, collectorList: InsulatorCollector[], beamCollector: InsulatorCollector, dataMap: DataMap): void {
