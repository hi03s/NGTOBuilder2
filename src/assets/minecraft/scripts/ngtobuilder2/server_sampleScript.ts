...
function onUpdate2(entity: EntityVehicle, scriptExecuter: ScriptExecuter): void {
    ...
    //生成
    const receiveData = NGTOBuilderUtil.getJsonData<ReceiveData_test>(dataMap, "sendData");
    ...
    if (receiveData) {
        ...
        if (!isInitializedBuild) {
            ...
            const blockSet = new BlockSet(Blocks.stone, 0);
            for (let i = 0; i < receiveData.pos.length; i++) {
                builder.add(entity, blockSet, receiveData.pos[i][0], receiveData.pos[i][1], receiveData.pos[i][2]);
            }
