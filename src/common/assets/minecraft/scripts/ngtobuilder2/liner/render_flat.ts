...
let lookingLine: Parts;
let selectedLineArrow: Parts;
let lookingLineArrow: Parts;
let selectedLineArrowF: Parts;
let lookingLineArrowF: Parts;
...
function renderForToolUser(entity: EntityVehicle, pass: number, par3: number): void {
      ...
      for (let bezierIdx = 0; bezierIdx < bezierList.length; bezierIdx++) {
        ...
        for (let idx = 0; idx <= split; idx++) {
          ...
          const topRBO = isHuge
            ...
            : RotatableBlockObjectFactory.createBox(new BlockSet(RTMApiCompat.getBlockStone(), 0), boxWidth, 1, 1);
  ...
  if (railMapCollector.size(entity) > 0) {
    ...
    for (let rmIdx = 0; rmIdx < railMapList.length; rmIdx++) {
      ...
      if (isReverse)
        NGTOBuilderUtilClient.renderRailMapStatic(renderer, selectedLineArrowF, rm, 10);
  ...
  if (lookingPos) {
    if (railMapCollector.size(entity) > 0) {
      ...
      if (lookingRailMap && !railMapCollector.hasRailMap(entity, lookingRailMap)) {
        ...
        if (keyManager.downOptionKey())
          NGTOBuilderUtilClient.renderRailMapStatic(
            renderer,
            lookingLineArrowF,
            lookingRailMap,
            10
          ...
          NGTOBuilderUtilClient.renderRailMapStatic(renderer, lookingLineArrow, lookingRailMap, 10);
        GL11.glPopMatrix();
      }
    } else if (posCollector.size(entity) > 0) {
      //## 通常描画
