...
function keyInput(
	...
	if (lookingPos && isRightClick) {
		...
		if (railMapCollector.size(entity) > 0) {
			...
			if (
				lookingRailMap &&
				!railMapCollector.hasRailMap(entity, lookingRailMap)
			) {
				railMapCollector.add(
					entity,
					lookingRailMap,
					keyManager.downOptionKey(),
				);
				bezierCollector.addFromRailMap(
					entity,
					lookingRailMap,
					keyManager.downOptionKey(),
				);
	...
	if (keyManager.pressed("isMasking")) {
		...
		NGTLog.sendChatMessage(
			sender,
			`[NGTO Builder2] ブロック置換: ${isMasking}`,
		);
	}
	...
	if (heldNGTO && bezierCollector.size(entity) > 0) {
		...
		if (!posList || posList.length === 0) {
			...
			if (isMasking)
				...
				posList = RotatableBlockObjectMapper.getPosList(
					margedRBO,
					origin[0],
					origin[1],
					origin[2],
