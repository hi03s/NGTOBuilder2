...
function keyInput(
  ...
  const beamConnectorModelName =
    insulatorItems && insulatorItems.length > 0
      ? RTMApiCompat.getModelNameFromItem(insulatorItems[insulatorItems.length - 1])
      : "NoModel_Side";
  ...
  if (
    lookingPos &&
    isRightClick &&
    (!heldItem || (heldItem && ignoreItemList.indexOf(heldItem.getItem()) === -1))
  ) {
  ...
  if (keyManager.pressed("laneLeft")) {
    ...
    const laneDescription =
      laneCount > 0 ? `R+${laneCount}` : laneCount < 0 ? `L+${Math.abs(laneCount)}` : "0";
    ...
    if (lookingRailMap) {
      ...
      if (collectorList[0].size(entity) > 0) {
        ...
        if (prevPos) {
          ...
          const toPlayerYaw =
            Math.atan2(posX - currentPos[0], posZ - currentPos[2]) * (180 / Math.PI) + 180;
          GL11.glPushMatrix();
          GL11.glTranslatef(
            rmPosZX[1] + deviationOffsetVec.getX(),
...
function renderBlockPreview(
  ...
  GL11.glPushMatrix();
  GL11.glTranslatef(Math.floor(x) + 0.5, Math.floor(y) + 0.5, Math.floor(z) + 0.5);
