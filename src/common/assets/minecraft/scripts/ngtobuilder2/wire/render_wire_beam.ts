...
function keyInput(
	...
	if (keyManager.pressed("showHelp")) {
		...
		NGTLog.sendChatMessage(
			sender,
			keyManager.getDescription("xOffsetIncrease"),
		);
		NGTLog.sendChatMessage(
			sender,
			keyManager.getDescription("xOffsetDecrease"),
		);
		NGTLog.sendChatMessage(
			sender,
	...
	//座標を追加
	const heldItem: ItemStack | null = NGTOBuilderUtil.getHeldItem(hostPlayer);
	if (
		lookingPos &&
		isRightClick &&
		(!heldItem ||
	...
	if (keyManager.pressed("insulatorHeightIncrease")) {
		...
		baseCollector.translateY(entity, 1);
		rebuildBeam(entity, baseCollector, beamCollector, dataMap);
		NGTLog.sendChatMessage(
			sender,
	...
	//ローカルXオフセットを増やす
	if (
		keyManager.pressed("xOffsetIncrease") ||
		keyManager.held("xOffsetIncrease", 500)
	) {
		xOffset += 0.1;
	...
	//ローカルXオフセットを減らす
	if (
		keyManager.pressed("xOffsetDecrease") ||
		keyManager.held("xOffsetDecrease", 500)
	) {
		xOffset -= 0.1;
		dataMap.setDouble("xOffset", xOffset, 0);
		rebuildBeam(entity, baseCollector, beamCollector, dataMap);
...
function getBeamInsulatorName(player: EntityPlayer): string {
	...
	if (insulatorItems.length > 0)
		modelName = RTMApiCompat.getModelNameFromItem(insulatorItems[0]);
	return modelName;
}
...
function rebuildBeam(
	entity: EntityVehicle,
	baseCollector: InsulatorCollector,
