...
function onUpdate2(entity: EntityVehicle, scriptExecuter: ScriptExecuter): void {
    ...
    if (isMarkerFix) {
        ...
        const posList: number[][] = posListData ? JSON.parse(posListData.replace(/☆/g, ",")) : [];
        if (posList.length > 0) {
    ...
    //生成
    const receiveData = NGTOBuilderUtil.getJsonData<ReceiveData_measure>(dataMap, "sendData");
    ...
    if (receiveData) {
        ...
        if (!isInitializedBuild && heldBlockSet) {
            ...
            //RBO生成
            const baseRbo = RotatableBlockObjectFactory.createBox(heldBlockSet, 1, 1, 1);
            //ベジェ曲線上に展開してRBOを合成する
            ...
            RotatableBlockObjectMapper.applyDiffusionSelf(margedRBO, BlockDiffusionMode.get(BlockDiffusionMode.XYZ));
