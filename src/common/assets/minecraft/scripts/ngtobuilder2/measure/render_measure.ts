...
function keyInput(
	...
	if (keyManager.pressed("showHelp")) {
		...
		NGTLog.sendChatMessage(
			sender,
			`[${Keyboard.getKeyName(keyManager.getOptionKeyCode())} + 右クリック] 座標を選択(スナップ)`,
		);
	...
	if (lookingPos && isRightClick) {
		...
		const lookingPosX = isSnap
			? Math.round(lookingPos.posX * 2) / 2
			: lookingPos.posX;
		const lookingPosY = isSnap
			? Math.round(lookingPos.posY * 2) / 2
			: lookingPos.posY;
...
//## グローバル変数として使うための準備 ##
let body: Parts;
let body_dismount: Parts;
let point: Parts;
let selected: Parts;
let distanceNum: Parts[];
let distance_M: Parts;
let distance_SUM: Parts;
let distance_ANGLE: Parts;
let distance_DECIMAL: Parts;
let distance_L: Parts;
let distance_COLON: Parts;
let line: Parts;
let line_selected: Parts;
let scale1: Parts;
let scale1_selected: Parts;
let scale10: Parts;
let scale10_selected: Parts;
let blockFrame: Parts;
...
function renderForToolUser(
	...
	if (lookingPos) {
		...
		if (posList.length > 0) {
			...
			const distanceVec = new Vec3(
				lookingPosX - prevPos[0],
				lookingPosY - prevPos[1],
				lookingPosZ - prevPos[2],
			);
			const distance = distanceVec.length();
			const yaw = distanceVec.getYaw();
			const pitch = distanceVec.getPitch();
			const toPlayerVec = new Vec3(
				player.posX - lookingPosX,
				player.posY - lookingPosY,
				player.posZ - lookingPosZ,
			);
			...
			for (let i = 0; i < distanceStr.length; i++) {
				...
				if (char >= "0" && char <= "9")
					distanceNum[Number(char)].render(renderer);
