...
import { ModelSetBase, ModelSetVehicleBase } from "jp.ngt.rtm.modelpack.modelset";
...
import { RailConfig, VehicleBaseConfig, VehicleConfig } from "jp.ngt.rtm.modelpack.cfg";
  ...
  static renderRailMapHighlight(
    ...
    if (tile instanceof TileEntityLargeRailBase && tile.getRailCore()) {
      ...
      if (!cachedData || cachedData[1] !== modelName) {
        ...
        const modelSet = RTMApiCompatClient.getRailModelSet(railCore) as ModelSetBase<RailConfig>;
        ...
        for (let rmIdx = 0; rmIdx < split; rmIdx++) {
          ...
          for (let groupIdx = 0; groupIdx < groupObjects.size(); groupIdx++) {
            ...
            if (
              shouldRenderObject &&
              (shouldRenderObject.invoke(railRenderer, railCore, name, len, pos) as boolean)
            ) {
