...
export class NGTOBuilderUtil {
   ...
   * @returns
   */
  static toJavaObjectArray<T>(array: T[], componentClass: java.lang.Class): JavaObjectArray<T> {
  ...
  static findMethod(clazz: Class, names: string[], paramTypes: Class[] = []): Method | null {
    ...
    while (current) {
      for (let i = 0; i < names.length; i++) {
        const name = names[i];
        try {
          const method = current.getDeclaredMethod(name, Java.to(paramTypes, "java.lang.Class[]"));
