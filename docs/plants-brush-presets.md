...
別のRTMモデルパックから、NGTO Builder 2の草木ブラシへプリセットを追加できます。
通常のモデルパックと同様にmodsフォルダに入れることで有効化できます。

[サンプルパック](../samples/plants-brush-preset-pack/)では、NGTZとNGTOを同じプリセットから読み込む構成を確認できます。

## ファイル構成

設定ファイルのファイル名を`*.ngtobplants.json`とし、NGTZまたはNGTOと同じフォルダへ配置してください。

```text
ExternalPlantsPack.zip
└─ assets/minecraft/scripts/ngtobuilder2/brush/presets/
   ├─ example_forest.ngtobplants.json
   ├─ example_forest.ngtz
   └─ single_tree.ngto
```
...
```json
{
	...
	"id": "example:forest",
	"name": "追加の森林",
	"blocks": ["example_forest.ngtz", "single_tree.ngto"],
	"modid": ["ExampleMod"],
	"randomHeight": true,
	"order": 1000
}
...
- 草木は生成時に0度、90度、180度、270度のいずれかへランダムに回転します。向きに依存するブロックや左右非対称の形状でも問題がないことを確認してください。
...
- クライアントとサーバーの内容が一致しない場合、誤生成防止のため草木の生成と消去が無効になります。
